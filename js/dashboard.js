...
Object.entries(profile.reminders || {}).forEach(([key, value]) => {
  ...
  if (timeDiff < 0) {
    ...
    message = `<strong>${label}:</strong> was due on ${reminderDate.toLocaleString()} <button class="deleteReminderButton" data-profile-index="${index}" data-reminder="${key}">🗑 Delete</button>`;

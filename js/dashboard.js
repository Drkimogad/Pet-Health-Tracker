...
const emergencyContact = profile.emergencyContacts?.[0] || {};
  ...
  const detailsHtml = `
    ...
    <div class="details-grid">
     ...
     <div>${getLastActivity(profile.id).replace('activity-section', '')}</div>
...
// 🔸 Delete locally (localStorage + memory)
let savedProfiles = JSON.parse(localStorage.getItem('petProfiles')) || [];
savedProfiles = savedProfiles.filter(p => p.id !== petId);
localStorage.setItem('petProfiles', JSON.stringify(savedProfiles));
...
showDashboardLoader(false, "success-deleting");
  ...
  // ================================
  // 📴 OFFLINE DELETION FLOW - FIXED SECTION
  // ================================
...
showDashboardLoader(true, "exporting");
    ...
    for (const profile of profiles) {
      ...
      petCard.innerHTML = `
       <div class="pet-card" style="border: 2px solid var(--accent-color); border-radius: 12px; padding-left: 20px;">
